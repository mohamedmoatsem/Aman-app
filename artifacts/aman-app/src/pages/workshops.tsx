...
export default function Workshops() {
  ...
  const dateLocale = w.dateLocale === "arSA" ? arSA : enUS;
  ...
  const formatDate = (dateString: string) => {
    try {
      ...
      return {
        ...
        full: format(date, w.dateFormat, { locale: dateLocale }),
        time: format(date, "h:mm a", { locale: dateLocale }),
      };
    } catch {
  ...
  return (
    <MobileLayout>
      <Header title={w.pageTitle} />

      <div className="px-4 py-6 flex flex-col gap-5">
        <p className="text-muted-foreground px-2 text-sm leading-relaxed">{w.subtitle}</p>
        ...
        {error && (
          <div className="p-6 bg-destructive/10 text-destructive rounded-3xl flex flex-col items-center justify-center text-center gap-3">
            ...
            <p className="font-medium">{w.error}</p>
          </div>
        )}
            ...
            return (
              ...
              >
                ...
                <div className="flex flex-col flex-1 py-1">
                  <h3 className="font-bold text-foreground text-base leading-tight mb-2">{workshop.title}</h3>
                  <p className="text-sm text-muted-foreground line-clamp-2 mb-4 leading-relaxed">{workshop.description}</p>
        ...
        {workshops?.length === 0 && (
          <div className="py-16 flex flex-col items-center justify-center text-center opacity-50">
            ...
            <p className="font-medium">{w.empty}</p>
          </div>
        )}
      </div>
    </MobileLayout>
